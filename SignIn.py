...
class SignInWidget(QWidget):
    ...
    is_expert_signal = pyqtSignal()
    is_worker_signal = pyqtSignal()

    def __init__(self):
        super(SignInWidget, self).__init__()
        self.resize(900, 600)
        self.setWindowTitle("欢迎使用Weldetect")
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label = QLabel("Weldetect焊件检测系统")
        fontlabel = QFont()
        ...
        if (not query.next()):
            ...
            if (userId == query.value(0) and hl.hexdigest() == query.value(2)):
                ...
                if (query.value(3)==2):
                    ...
                    self.is_expert_signal.emit()
                else:
                    self.is_worker_signal.emit()
            else:
